...
from load_data import load_data_from_npz, load_batch, load_data_names, load_batch_from_names_random, load_batch_reduce
...
def generator_reduce_npz(data, batch_size, img_ch, img_cols, img_rows):

    while True:
        for it in list(range(0, data[0].shape[0], batch_size)):
            x, y = load_batch_reduce([l[it:it + batch_size] for l in data], img_ch, img_cols, img_rows)
            yield x, y

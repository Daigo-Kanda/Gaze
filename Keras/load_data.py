...
import json
from Keras.data_utility import image_normalization
...
def load_batch(data, img_ch, img_cols, img_rows):
    ...
    for i, img in enumerate(data[0]):
        ...
        if save_images:
            cv2.imwrite(join(img_dir, "left" + str(i) + ".png"), cv2.cvtColor(img, cv2.COLOR_RGB2BGR))
    ...
    for i, img in enumerate(data[1]):
        img = cv2.resize(img, (img_cols, img_rows))
        if save_images:
            cv2.imwrite("images/right" + str(i) + ".png", cv2.cvtColor(img, cv2.COLOR_RGB2BGR))
        img = image_normalization(img)
    ...
    for i, img in enumerate(data[2]):
        img = cv2.resize(img, (img_cols, img_rows))
        if save_images:
            cv2.imwrite("images/face" + str(i) + ".png", cv2.cvtColor(img, cv2.COLOR_RGB2BGR))
        img = image_normalization(img)
        face_batch[i] = img

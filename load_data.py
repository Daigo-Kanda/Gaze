...
# load a batch with data loaded from the npz file
def load_batch_reduce(data, img_ch, img_cols, img_rows):
    ...
    return [right_eye_batch, face_batch, face_grid_batch], y_batch
...
# 特定の一人のデータだけを返却する．
"""
def load_data_names(path, num):

    seq_list = []
    seqs = sorted(glob.glob(join(path, num)))
